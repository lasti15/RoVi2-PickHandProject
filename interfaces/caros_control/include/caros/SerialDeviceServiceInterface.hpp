...
namespace caros {
    /* TODO:
     * Should this interface also contain a cleanupInterface (or private cleanupService() and public deconfigureInterface() or shutdownInterface()) ?
     * ^- The GripperServiceInterface contains these sort of functions
     * ^^-- But the interfaces could also just be converted to RAII style and have them enabled by default without the option to switch them off - ros::spin[all] (or similar) has to be called anyway for the processing of the service requests, so it's not like the services can be called before the node is ready to process them anyway.
     */
    ...
    public:
         ...
         * ^- Also look at the return/response fields and make sure they are streamlined (currently they should only return a boolean based on the success of the ROS service call (not whether the actually action has been done or not - just that it was a valid action (i.e. same amount of targets and speeds, and that IK solutions could be found).
         */
        ...
        virtual bool moveLinFC(const rw::math::Transform3D<>& posTarget, const rw::math::Transform3D<>& offset, const rw::math::Wrench6D<>& wrenchTarget, const rw::math::Q& controlGain) = 0;

        /* TODO:
         * Does it make sense that the servoQ and servoT are taking in multiple targets, instead of 1 target for each invocation?
         * As far as I understand it, then servoing is supposed to just move to the newest configuration and throw away all the intermediate configurations as they are irrelevant and won't be used as waypoints.
         */
        ...
        /* movePause should just (temporarily) pause the movement, and allow for it to continue (i.e. the robot should still be able to perform the full trajectory) */
