 ...
 *  ^-- If using a thread to run the handleGripperState callback, then it would make sense to expose the reported gripperState to the user or create a struct holding the same fields just with converted types such as rw::math::Q instead of caros_common/Q.
 *
    ...
    _nodeHnd(nodehandle)
{
    ...
    _srvMoveQ = _nodeHnd.serviceClient<caros_control::GripperMoveQ>(rosNamespace.str() + "/move_q");
    _srvGripQ = _nodeHnd.serviceClient<caros_control::GripperGripQ>(rosNamespace.str() + "/grip_q");
    _srvSetForceQ = _nodeHnd.serviceClient<caros_control::GripperSetForceQ>(rosNamespace.str() + "/set_force_q");
    _srvSetVelocityQ = _nodeHnd.serviceClient<caros_control::GripperSetVelocityQ>(rosNamespace.str() + "/set_velocity_q");
    _srvStopMovement = _nodeHnd.serviceClient<caros_control::GripperStopMovement>(rosNamespace.str() + "/stop_movement");
...
bool GripperSIProxy::moveQ(const rw::math::Q& q)
{
    ...
    caros_control::GripperMoveQ srv;
...
{
    bool srvCallSuccess = false;
    caros_control::GripperGripQ srv;
    srv.request.q = caros::toRos(q);
...
{
    bool srvCallSuccess = false;
    caros_control::GripperSetForceQ srv;
...
{
    bool srvCallSuccess = false;
    caros_control::GripperSetVelocityQ srv;
...
{
    bool srvCallSuccess = false;
    caros_control::GripperStopMovement srv;
...
void GripperSIProxy::handleGripperState(const caros_control::GripperState& state) {
	boost::mutex::scoped_lock lock(_pSV);
