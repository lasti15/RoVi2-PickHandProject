...
namespace caros {
    ...
    bool GripperServiceInterface::initGripperService() {
        ...
        _gripperStatePublisher = _nodeHandle.advertise<caros_control::GripperState>("GripperState", GRIPPER_STATE_PUBLISHER_QUEUE_SIZE);
        ...
        if (_gripperStatePublisher && _srvMoveQ && _srvGripQ && _srvSetForceQ && _srvSetVelocityQ && _srvStopMovement) {
            ...
            ROS_ERROR_STREAM("The gripper service could not be properly initialised - one or more ros services or publishers failed to be properly initialised.");
            return false;
        }
