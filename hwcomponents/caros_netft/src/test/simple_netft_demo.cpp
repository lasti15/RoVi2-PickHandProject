...
int main(int argc, char* argv[])
{
  ...
  const std::string node_state_topic_name = node_under_test_name + "/" + "caros_node/caros_node_state";
  bool is_running = false;
  while (!is_running)
  {
    auto node_state_message = ros::topic::waitForMessage<caros_common_msgs::CarosNodeState>(node_state_topic_name);
