...
bool doCollisionDetectionPrototype(const rw::math::Q &startConfiguration, const rw::math::Q &endConfiguration)
{
  ...
  {
    ROS_FATAL_STREAM("The parameter '" << nodehandle_.getNamespace() << "/deviceName' was not present on the parameter "
  ...
  /* TODO:
   * [ IMPORTANT ] Has to know the state of the other UR (UR2 also) in its live configuration, and not the configuration
   * that was hardcoded within the scene/workcell.
   */
  ...
  const rw::pathplanning::PlannerConstraint plannerConstraint =
      rw::pathplanning::PlannerConstraint::make(detector, device_, state_);
  ...
  {
    ROS_DEBUG_STREAM("The path between the start configuration '" << startConfiguration
                                                                  << "' and the end configuration '" << endConfiguration
                                                                  << "' is in collision!");
    return false;
  }
...
bool doCollisionFreeServoing(caros::SerialDeviceSIProxy &sdsip, const rw::math::Q &startConfiguration,
                             const rw::math::Q &endConfiguration)
{
  bool status = false;
...
int testUsingMoveServoQ(void)
{
  ...
  {
    ROS_FATAL_STREAM("The parameter '" << nh.getNamespace() << "/qChange' was not present on the parameter server! "
                                                               "This parameter has to be specified for this test-node "
                                                               "to work properly.");
    return -2;
  }
  ...
  {
    ROS_FATAL_STREAM("The parameter '" << nh.getNamespace() << "/steps' was not present on the parameter server! This "
                                                               "parameter has to be specified for this test-node to "
                                                               "work properly.");
    return -2;
  }
  ...
  {
    ROS_FATAL_STREAM("The parameter '" << nh.getNamespace() << "/movements' was not present on the parameter server! "
                                                               "This parameter has to be specified for this test-node "
                                                               "to work properly.");
    return -2;
  }
  ...
  /* FIXME:
   * Idle until the movement has finished - look at the different flags/changes on the node - can it be seen when the
   *node is going to move and then when it's finished moving
   * ^- The hard part seems to be the initial wait from sending the command until the receiving node has processed it
   *and changed its state...
   *
